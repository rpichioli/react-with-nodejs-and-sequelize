...
class AlbumFormPage extends React.Component {
	...
	handleSubmit = (e) => {
		...
		if (Object.keys(errors).length === 0) {
			...
			if (this.props.album) {
				this.props.updateAlbum(this.state).then(() => { this.setState({ redirect: true }) });
			} else {
				this.state.band_id = this.props.match.params.id
				this.props.addAlbum(this.state);//.then(() => { this.setState({ redirect: true }) });
